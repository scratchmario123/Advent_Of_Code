    ...
    public static void main(String[] args) {
        try {
            ...
            while (scanner.hasNext()) {
                ...
                int wins = winning.stream().filter(e -> Arrays.stream(s.split(": ")[1].split(" \\| ")[1].trim().replaceAll(" {2}"," ").split(" ")).map(Integer::parseInt).toList().contains(e)).toList().size();
            ...
            sum = copies.subList(0,cards).stream().mapToInt(Integer::intValue).sum();
